...
@media (max-width: 768px) {
  /* app layout — 总高度 可变 (header 3-row 后 不再 fixed 52px) */
  .app { grid-template-rows: auto 26px 1fr; min-height: 100vh; height: auto; }

  /* header — 改 3-row grid layout: logo+status / tabs / actions
     沈继元 撤回 PASS 实测 发现 5 个 bug 全 在 header 这一 层 */
  .header {
    display: grid;
  ...
  .logo { grid-area: logo; white-space: nowrap; flex-shrink: 0; min-width: 0; font-size: 14px; }
  .logo-mark { width: 20px; height: 20px; font-size: 12px; }
  .logo-text { white-space: nowrap; }
  .logo-sub { display: none; }
  .logo-sep { display: none; }

  /* status pill — "未配置" 3 字 强制 single line */
  .status {
    grid-area: status;
    white-space: nowrap;
    flex-shrink: 0;
  ...
  /* view tabs (顶层 dashboard/reader) — 全 row 2, horizontal scroll */
  .view-tabs {
    grid-area: tabs;
    display: flex;
    gap: 4px;
  ...
  /* header actions — 4 按钮 排 row 3, flex-wrap */
  .header-actions {
    grid-area: actions;
    display: flex;
    align-items: center;
  ...
  /* dash tabs (dashboard 内 5 sub-tab) — horizontal scroll */
  .dash-tabs {
    padding: 0 10px;
    overflow-x: auto;
    -webkit-overflow-scrolling: touch;
  ...
  .dash-tab { padding: 8px 11px; font-size: 11.5px; flex-shrink: 0; white-space: nowrap; }

  /* dash pane content — 全宽 100%, no max-width */
  .dash-pane-inner { padding: 14px 14px 40px; max-width: 100%; }
  .dash-pane-inner.active { max-width: 100%; margin: 0; }

  /* KPI grid — vertical 1 column, full width */
  .kpi-grid { grid-template-columns: 1fr; gap: 10px; margin-bottom: 16px; }
  .health-grid { grid-template-columns: 1fr; gap: 10px; }

  /* KPI cards — full width (width 100% + min-width 0 防 挤压 overflow) */
  .kpi {
    width: 100%;
    min-width: 0;
  ...
  .kpi-meta, .kpi-sub { font-size: 10.5px; }
  ...
  /* entity review */
  .entity-review-wrap { padding: 14px 12px 40px; max-width: 100%; }
  ...
  .entity-card-actions { gap: 4px; flex-wrap: wrap; }
  .entity-card-actions .btn { font-size: 11px; padding: 3px 7px; height: 26px; flex: 1 1 auto; }
...
@media (max-width: 480px) {
  ...
  .header { padding: 4px 10px; gap: 3px 6px; }

  .logo { font-size: 13px; gap: 7px; }
  .logo-mark { width: 18px; height: 18px; font-size: 11px; }

  .status { padding: 2px 7px; font-size: 10.5px; }

  .view-tab { padding: 6px 9px; font-size: 10.5px; }
  .view-tab .dot { width: 4px; height: 4px; }

  .dash-tab { padding: 7px 9px; font-size: 11px; }

  .header-actions .btn { padding: 3px 7px; font-size: 10.5px; height: 24px; }
