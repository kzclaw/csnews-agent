...
describe('wrangler.toml · 两个 worker 关键字段 guard', () => {
  describe('csnews-agent/wrangler.toml', () => {
    ...
    // v0.37.18 (方案 4 拍板): WORKER_VERSION 字段加回 — 但值由顶层 .husky/pre-commit hook 自动
    // bump tag (v0.37.x+1) · board 17:59 拍板. 跟 v0.37.17 KV 注入方案说再见.
    // tag 在 post-commit hook 立刻 git tag 绑回 commit · wrangler.toml 字段 = 当前 commit 的 tag 名.
    it('v0.37.18 (方案 4): 必须包含 WORKER_VERSION 字段, 值匹配 v{major}.{minor}.{patch} tag 格式', () => {
      expect(/^WORKER_VERSION\s*=\s*"v\d+\.\d+\.\d+"/m.test(toml)).toBe(true);
    });
  });
