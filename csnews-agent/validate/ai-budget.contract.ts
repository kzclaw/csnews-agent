...
describe('shouldTriggerAiCall — L1-L3 unrestricted', () => {
  it('L1 always returns true', async () => {
    await expect(shouldTriggerAiCall({}, 'L1')).resolves.toBe(true);
  });

  it('L2 always returns true', async () => {
    await expect(shouldTriggerAiCall({}, 'L2')).resolves.toBe(true);
  });

  it('L3 always returns true', async () => {
    await expect(shouldTriggerAiCall({}, 'L3')).resolves.toBe(true);
  });
});

describe('shouldTriggerAiCall — L4-L6 sync stub', () => {
  it('L4 returns true (sync stub — Phase 2 scope)', async () => {
    await expect(shouldTriggerAiCall({}, 'L4')).resolves.toBe(true);
  });

  it('L5 returns true (sync stub — Phase 2 scope)', async () => {
    await expect(shouldTriggerAiCall({}, 'L5')).resolves.toBe(true);
  });

  it('L6 returns true (sync stub — Phase 2 scope)', async () => {
    await expect(shouldTriggerAiCall({}, 'L6')).resolves.toBe(true);
  });

  it('accepts severity parameter without error', async () => {
    await expect(shouldTriggerAiCall({}, 'L4', 8)).resolves.toBe(true);
  });

  it('accepts dailyUsed parameter without error', async () => {
    await expect(shouldTriggerAiCall({}, 'L4', undefined, 6000)).resolves.toBe(true);
  });
});
...
describe('Integration — recordAiCall + getBudgetStatus round-trip', () => {
  ...
  it('shouldTriggerAiCall still allows L1-L3 even at shutdown', async () => {
    const env = makeEnv();
