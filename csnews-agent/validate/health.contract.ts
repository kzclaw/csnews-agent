...
describe('health-ai.ts — checkAiCallsBreakdown', () => {
  ...
  it('aggregates calls by L1/L3/L6 tier levels', async () => {
    ...
    store[todayKey] = JSON.stringify({
      total: 1000,
      calls: [
        { model: '@cf/meta/llama-3.1-8b-instruct-fp8', neurons: 100, tool_name: 'tool1' },
        { model: '@cf/baai/bge-m3', neurons: 200, tool_name: 'tool2' },
        { model: '@cf/meta/llama-3.1-8b-instruct-fp8', neurons: 150, tool_name: 'tool1' },
    ...
    // L6 for llama-3.1, L3 for bge-m3, L1 for unknown
...
describe('health-r2.ts — checkR2LatestWrite', () => {
  ...
  it('empty R2 bucket returns null without crash', async () => {
    ...
    expect(result.r2_latest_write).not.toBeNull();
    expect(result.r2_latest_write).toHaveProperty('empty', true);
    expect(result.r2_latest_write).toHaveProperty('r2_role', 'cold_archive');
    expect(result.checks.r2_latest_write.status).toBe('info');
  ...
  it('R2 list throws returns error without crash', async () => {
    ...
    expect(result.checks.r2_latest_write.status).toBe('info'); // cold archive, never escalates above 'info'
  });
});
