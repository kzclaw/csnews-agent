...
- [ ] T000 [P] [US1] Create `csnews-agent/src/auth.ts` — `authRequest()` + `corsHeaders()`（~20 行）
- [ ] T000 [P] [US1] Create `csnews-agent/src/classify.ts` — `CATEGORY_KW` + `classifyRule()` + `classifyByAI()` + `classify()`（~90 行）
- [ ] T000 [P] [US1] Create `csnews-agent/src/score.ts` — `scoreRule()` + 路由常量（`AI_ROUTE_R_THRESHOLD` / `TOPIC_MATCH_THRESHOLD` / `R2_DUP_THRESHOLD`）（~30 行）
